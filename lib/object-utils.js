...
(function (isNode) {
    ...
    function addDerivedInfoForFile(fileCoverage) {
        ...
        if (!fileCoverage.l) {
            ...
            Object.keys(statements).filter(function (st) {
                return statementMap[st];
            }).forEach(function (st) {
    ...
    function computeSimpleTotals(fileCoverage, property, mapProperty) {
        ...
        Object.keys(stats).forEach(function (key) {
            var covered = !!stats[key],
                skipped = map && map[key] && map[key].skip;
            ret.total += 1;
            if (covered || skipped) {
                ret.covered += 1;
            }
    ...
    function computeBranchTotals(fileCoverage) {
        ...
        Object.keys(stats).forEach(function (key) {
            ...
            for (i = 0; i < branches.length; i += 1) {
                ...
                skipped = map && map.locations && map.locations[i] && map.locations[i].skip;
                if (covered || skipped) {
                    ret.covered += 1;
                }
                if (!covered && skipped) {
